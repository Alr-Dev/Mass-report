# roblox updated api, its the new script
...
def report_game(session, game_id, report_details, csrf_token, cookie_string):
    """
    Reports a game using the abuse report API endpoint.
    """
    headers = {
    ...
    try:
        while True:
            for selected_cookie in cookies:
                ...
                reported = False
                while not reported:
                    reported = report_game(session, game_id, "This is not good content", csrf_token, cookie_string)
...
def main():
    ...
    print(Colorate.Vertical(Colors.purple_to_blue, "Version 0.0.4"))
    print(Colorate.Vertical(Colors.purple_to_blue, "Type: abuse report"))
